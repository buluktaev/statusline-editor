...
function generateScript() {
  ...
  if (hasRate) {
    ...
    lines.push('');
    lines.push('# Detect Python interpreter (cross-platform: python3 / python / py)');
    lines.push('PYTHON=""');
    lines.push('for _p in python3 python py; do');
    lines.push('    if command -v "$_p" >/dev/null 2>&1; then PYTHON="$_p"; break; fi');
    lines.push('done');
    lines.push('');
    lines.push('# Powershell call for Windows Credential Manager (used by Git Bash/MSYS/Cygwin/WSL)');
    lines.push('_read_cred_windows() {');
    lines.push('    powershell.exe -NoProfile -Command \\');
    lines.push('        \'[System.Text.Encoding]::UTF8.GetString([System.Convert]::FromBase64String((Get-StoredCredential -Target "Claude Code-credentials" -AsCredentialObject).Password))\' 2>/dev/null | tr -d \'\\r\'');
    lines.push('}');
    lines.push('');
    lines.push('fetch_usage() {');
    lines.push('    local token cred_json');
    lines.push('    [ -z "$PYTHON" ] && return');
    lines.push('    case "$(uname -s)" in');
    lines.push('        Darwin)');
    lines.push('            cred_json=$(security find-generic-password -s "Claude Code-credentials" -w 2>/dev/null)');
    lines.push('            ;;');
    lines.push('        Linux)');
    lines.push('            if grep -qi microsoft /proc/version 2>/dev/null; then');
    lines.push('                cred_json=$(_read_cred_windows)');
    lines.push('            else');
    lines.push('                cred_json=$(secret-tool lookup service "Claude Code-credentials" 2>/dev/null)');
    lines.push('            fi');
    lines.push('            ;;');
    lines.push('        *MINGW*|*MSYS*|*CYGWIN*)');
    lines.push('            cred_json=$(_read_cred_windows)');
    lines.push('            ;;');
    lines.push('    esac');
    lines.push('    [ -z "$cred_json" ] && return');
    lines.push('    token=$(echo "$cred_json" | "$PYTHON" -c "');
    lines.push('import sys, json, binascii');
    lines.push('data = sys.stdin.buffer.read().strip()');
    lines.push('try:');
    lines.push("    raw = binascii.unhexlify(data)");
    lines.push('except Exception:');
    lines.push("    raw = data");
    lines.push("idx = raw.find(b'{')");
    lines.push('if idx >= 0:');
    lines.push("    d, _ = json.JSONDecoder().raw_decode(raw[idx:].decode('utf-8', errors='replace'))");
    lines.push("    t = d.get('accessToken') or d.get('claudeAiOauth', {}).get('accessToken', '')");
    lines.push('    print(t)');
    ...
    lines.push('');
    lines.push('    five_h_left=$("$PYTHON" -c "import sys; print(int(100 - float(sys.argv[1])))" "$five_h_used" 2>/dev/null || echo "?")');
    lines.push('    week_left=$("$PYTHON"   -c "import sys; print(int(100 - float(sys.argv[1])))" "$week_used"   2>/dev/null || echo "?")');
    lines.push('');
    lines.push("    [ -n \"$five_h_reset_at\" ] && time_left=$(\"$PYTHON\" -c \"");
    ...
    lines.push('');
    lines.push("    [ -n \"$week_reset_at\" ] && week_reset_str=$(\"$PYTHON\" -c \"");
    lines.push('import sys');
    lines.push('from datetime import datetime, timezone');
    lines.push("reset = datetime.fromisoformat(sys.argv[1].replace('Z','+00:00'))");
    lines.push('s = int((reset - datetime.now(timezone.utc)).total_seconds())');
