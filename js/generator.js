...
function generateScript() {
  ...
  if (hasRate) {
    ...
    lines.push('            cred_json=$(powershell.exe -NoProfile -Command \\');
    lines.push('                \'[System.Text.Encoding]::UTF8.GetString([System.Convert]::FromBase64String((Get-StoredCredential -Target "Claude Code-credentials" -AsCredentialObject).Password))\' 2>/dev/null)');
    lines.push('            ;;');
    lines.push('    esac');
    lines.push('    [ -z "$cred_json" ] && return');
    lines.push('    token=$(echo "$cred_json" | python3 -c "');
