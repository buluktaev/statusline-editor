...
const DEFAULTS = {
  ...
  blocks: [
    ...
    { id: 'git', name: 'Git', enabled: false, colorBranch: 254, showStatus: true, colorClean: 82, colorDirty: 203, showCounts: true, colorStaged: 82, colorModified: 226, colorUntracked: 245 },
  ]
};
...
if (!state.blocks.find(b => b.id === 'git')) {
  state.blocks.push({ id: 'git', name: 'Git', enabled: false, colorBranch: 254, showStatus: true, colorClean: 82, colorDirty: 203, showCounts: true, colorStaged: 82, colorModified: 226, colorUntracked: 245 });
}
state.blocks.forEach(block => {
  if (block.id === 'git') {
    ...
    if (!('colorUntracked' in block)) block.colorUntracked = 245;
  }
});
